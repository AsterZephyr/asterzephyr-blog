...
<TechGraph src="/images/posts/幂等性没你想的那么简单-当第二个请求和第一个不同时/design-debate.svg" title="幂等设计两派之争" caption="极简派（一个 unique constraint）vs 完整 Replay 派（存储请求+响应+状态）" />
...
<TechGraph src="/images/posts/幂等性没你想的那么简单-当第二个请求和第一个不同时/concurrent-sequence.svg" title="INSERT-FIRST 并发时序" caption="INSERT ON CONFLICT 原子获取执行权，避免 check-then-insert 竞态" />
...
<TechGraph src="/images/posts/幂等性没你想的那么简单-当第二个请求和第一个不同时/state-machine.svg" title="幂等记录状态机" caption="完整状态机：6 种状态 + 重试到达时的行为分支" />
