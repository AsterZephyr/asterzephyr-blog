...
<TechGraph src="/images/posts/8张4090微调235b模型-roundpipe如何打破消费级gpu训练的天花板/traditional-vs-roundpipe.svg" title="传统 Pipeline vs RoundPipe" caption="传统方案 GPU 绑定固定 Stage 导致 30% bubble，RoundPipe 将 GPU 变为无状态执行池实现 bubble≈0" />
...
<TechGraph src="/images/posts/8张4090微调235b模型-roundpipe如何打破消费级gpu训练的天花板/pcie-transfer-scheduling.svg" title="优先级感知的 PCIe 传输调度" caption="激活值传输最高优先级（关键路径），参数预取和梯度回传填入空闲窗口，计算与传输完全重叠" />
...
<TechGraph src="/images/posts/8张4090微调235b模型-roundpipe如何打破消费级gpu训练的天花板/evolution-timeline.svg" title="消费级 GPU 训练技术演进" caption="ZeRO-Offload → LoRA → Mobius → RoundPipe，每代解决上代核心瓶颈，可行边界持续上移" />
