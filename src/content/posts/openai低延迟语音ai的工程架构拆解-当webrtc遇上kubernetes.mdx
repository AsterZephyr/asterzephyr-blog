...
<TechGraph src="/images/posts/openai低延迟语音ai的工程架构拆解-当webrtc遇上kubernetes/webrtc-k8s-conflicts.svg" title="WebRTC 和 Kubernetes 的核心矛盾" caption="端口爆炸、状态粘性、弹性受限——三大冲突" />
...
<TechGraph src="/images/posts/openai低延迟语音ai的工程架构拆解-当webrtc遇上kubernetes/relay-transceiver-arch.svg" title="Relay + Transceiver 分离架构" caption="将包路由（无状态）和协议终结（有状态）分离为两个独立服务" />
...
<TechGraph src="/images/posts/openai低延迟语音ai的工程架构拆解-当webrtc遇上kubernetes/ice-ufrag-routing.svg" title="ICE ufrag 首包路由流程" caption="路由信息编码在协议标准字段中，首包路由无需外部查找服务" />
...
<TechGraph src="/images/posts/openai低延迟语音ai的工程架构拆解-当webrtc遇上kubernetes/global-relay.svg" title="全球 Relay 和地理感知信令" caption="信令和媒体都走就近入口，会话锚定在特定 Transceiver" />
...
<TechGraph src="/images/posts/openai低延迟语音ai的工程架构拆解-当webrtc遇上kubernetes/go-implementation.svg" title="Go 实现：用户态优化" caption="SO_REUSEPORT + LockOSThread + 预分配 buffer，不需要内核旁路" />
...
<TechGraph src="/images/posts/openai低延迟语音ai的工程架构拆解-当webrtc遇上kubernetes/architecture-philosophy.svg" title="架构哲学：复杂性应该加在哪里" caption="薄路由层集中处理转换，让后端服务和客户端都保持纯粹" />
