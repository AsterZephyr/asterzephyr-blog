...
<TechGraph src="/images/posts/OneTrans-字节跳动如何用一个Transformer统一推荐系统的特征交互与序列建模/dual-vs-unified.svg" title="架构对比" caption="传统双模块架构 (信息瓶颈) vs OneTrans 统一架构 (每层交互)" />
...
<TechGraph src="/images/posts/OneTrans-字节跳动如何用一个Transformer统一推荐系统的特征交互与序列建模/scaling-evolution.svg" title="SCALING 演进" caption="分别 Scale → 发现叠加困境 → 统一架构突破瓶颈" />
...
<TechGraph src="/images/posts/OneTrans-字节跳动如何用一个Transformer统一推荐系统的特征交互与序列建模/learning-path.svg" title="学习路径" caption="从推荐基础 → Transformer 应用 → Scaling → 统一架构" />
