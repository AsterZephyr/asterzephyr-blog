...
<Diagram title="X FOR YOU 推荐系统架构" caption="四个组件的数据流与协作关系">
```mermaid
graph TB
    subgraph request["用户请求 For You 信息流"]
        REQ["Request"]
    end

    subgraph homemixer["Home Mixer - Rust 编排层"]
        QH["Query Hydration<br/>用户上下文加载"]
        SRC["Candidate Sources"]
        HYD["Candidate Hydration"]
        FLT["Filters"]
        SCR["Scoring Pipeline"]
        SEL["Top-K Selection"]
    end

    subgraph sources["候选源"]
        THD["Thunder<br/>关注者帖子<br/>Kafka + 内存"]
        PHR["Phoenix Retrieval<br/>全局 Two-Tower<br/>ANN 检索"]
    end

    subgraph scoring["打分体系"]
        PHS["Phoenix Scorer<br/>Grok Transformer"]
        WTS["Weighted Scorer<br/>19 行为加权"]
        ADS["Author Diversity<br/>同作者衰减"]
        OON["OON Scorer<br/>站外降权"]
    end
    ...
    style THD fill:#d3f9d8,stroke:#2f9e44
    style PHR fill:#e5dbff,stroke:#5f3dc4
    style PHS fill:#ffe8cc,stroke:#d9480f
```
</Diagram>
...
<Diagram title="CANDIDATE ISOLATION ATTENTION MASK" caption="候选帖子只能看到用户上下文，不能互相 attend">
```mermaid
graph LR
    subgraph mask["Attention Mask 结构"]
        direction TB
        M1["User + History 区域<br/>完全双向 Attention"]
        M2["Candidates 区域<br/>只能看 User+History<br/>不能看其他 Candidate"]
        M3["结果：每个 Candidate<br/>分数独立于 batch 组成"]
    end

    M1 --> M2
    M2 --> M3

    style M1 fill:#d3f9d8,stroke:#2f9e44
    style M2 fill:#ffe3e3,stroke:#c92a2a
    style M3 fill:#c5f6fa,stroke:#0c8599
```
</Diagram>
...
<Diagram title="数据规模差异决定了特征策略" caption="日活跃交互量对比（对数坐标）">
```mermaid
xychart-beta
    title "日活跃交互量对比 (log scale)"
    x-axis ["X 平台", "头部短视频", "中型 DSP", "垂直电商"]
    y-axis "日交互量 (log scale)" 6 --> 11
    bar [10.5, 10, 8, 7]
```
</Diagram>
...
<Diagram title="推荐系统架构代际演进" caption="X Phoenix 属于第三代，但通用性不如同代的 OneTrans 和 RankMixer">
```mermaid
graph TB
    subgraph gen1["第一代：手工特征加浅层模型"]
        FM["FM / DeepFM<br/>二阶交互 + MLP"]
        WD["Wide and Deep<br/>记忆 + 泛化"]
    end

    subgraph gen2["第二代：深度模型加分模块"]
        DIN["DIN/DIEN<br/>序列注意力"]
        DCN["DCN-v2<br/>显式高阶交叉"]
        PLE["PLE/MMOE<br/>多任务门控"]
    end

    subgraph gen3["第三代：Transformer 统一"]
        OT["OneTrans<br/>统一 Tokenizer + Causal"]
        XP["X Phoenix<br/>Candidate Isolation"]
        MX["RankMixer/TokenMixer<br/>Token Mixing 替代 Attention"]
    end

    subgraph gen4["第四代：端到端生成"]
        OR["OneRec<br/>Encoder-Decoder 生成"]
    end
    ...
    style XP fill:#ffe8cc,stroke:#d9480f
    style OT fill:#e5dbff,stroke:#5f3dc4
    style OR fill:#d3f9d8,stroke:#2f9e44
    style MX fill:#c5f6fa,stroke:#0c8599
```
</Diagram>
